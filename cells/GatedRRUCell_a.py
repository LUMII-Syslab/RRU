...
class RRUCell(LayerRNNCell):
    ...
    def build(self, inputs_shape):
        ...
        self._W_kernel = self.add_variable(
            ...
            shape=[n_middle_maps, self._num_units*2+self._output_size],
            initializer=self._kernel_initializer)
        self._W_bias = self.add_variable(
            "W/%s" % _BIAS_VARIABLE_NAME,
            shape=[self._num_units*2+self._output_size],
            initializer=self._bias_initializer)
        ...
        #     shape=(),  # (self._num_units)
        #     initializer=tf.constant_initializer(1.0))  # (0.25)
    ...
    def zero_state(self, batch_size, dtype):
        ...
        initial = onehot*np.sqrt(self._num_units)*0.25
